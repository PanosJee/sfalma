  ...
  class Remote
    class << self
      def startup_announce(startup_data)
        url = "/api/announcements?api_key=#{::Sfalma::Config.api_key}&protocol_version=#{::Sfalma::PROTOCOL_VERSION}"
      ...
      def error(exception_data)
        ...
        url = "/api/errors?api_key=#{::Sfalma::Config.api_key}&protocol_version=#{::Sfalma::PROTOCOL_VERSION}#{hash_param}"
